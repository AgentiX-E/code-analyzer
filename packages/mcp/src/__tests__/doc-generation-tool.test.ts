...
function createStoreWithData(): InMemoryGraphStore {
  ...
  const fId = insertNode(store, { projectId: 'test-project', label: 'Function', name: 'fetchData', qualifiedName: 'fetchData', filePath: 'src/api.ts' });
  const cId = insertNode(store, { projectId: 'test-project', label: 'Class', name: 'DataService', qualifiedName: 'DataService', filePath: 'src/api.ts' });
  const dId = insertNode(store, { projectId: 'test-project', label: 'Function', name: 'makeRequest', qualifiedName: 'makeRequest', filePath: 'src/http.ts' });
...
describe('docGenerationTool', () => {
  ...
  it('should support doc styles', () => {
    const e = (docGenerationTool.inputSchema.properties.style as Record<string, unknown>).enum as string[];
  ...
  it('should generate JSDoc', async () => {
    const r = await docGenerationTool.handler(
      { projectId: 'test-project', style: 'jsdoc', maxResults: 10 }, createStoreWithData(),
    );
  ...
  it('should generate docstring', async () => {
    const r = await docGenerationTool.handler(
      { projectId: 'test-project', style: 'docstring', maxResults: 10 }, createStoreWithData(),
    );
    expect(r.metadata.style).toBe('docstring');
  });
  it('should generate godoc', async () => {
    const r = await docGenerationTool.handler(
      { projectId: 'test-project', style: 'godoc', maxResults: 10 }, createStoreWithData(),
    );
  ...
  it('should respect maxResults', async () => {
    const r = await docGenerationTool.handler(
      { projectId: 'test-project', maxResults: 1 }, createStoreWithData(),
    );
