...
export async function exportReport(args: Record<string, unknown>, store?: unknown): Promise<ToolResult> {
  ...
  // Regenerate report content from store data to produce a real export
  let reportContent = '';
  let exported = false;
  let message = '';
  const exportMeta = { projectId: 'unknown', nodeCount: 0, edgeCount: 0, type: 'unknown' };
...
th{background:#f5f5f5}h1,h2{color:#333}</style></head>
<body>
<h1>Code Analyzer Report</h1>
<p><strong>Report ID</strong>: ${reportId}<br>
<strong>Generated</strong>: ${new Date().toISOString()}<br>
<strong>Project</strong>: ${exportMeta.projectId}</p>
<h2>Summary</h2>
<table><tr><th>Metric</th><th>Value</th></tr>
<tr><td>Total Nodes</td><td>${exportMeta.nodeCount}</td></tr>
<tr><td>Total Edges</td><td>${exportMeta.edgeCount}</td></tr>
</table>
<h2>Node Distribution</h2>
<table><tr><th>Label</th><th>Count</th></tr>${distributionRows}</table>
<h2>Top Complexity Symbols</h2>
