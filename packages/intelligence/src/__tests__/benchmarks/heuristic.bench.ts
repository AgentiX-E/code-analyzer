...
function createDiff() {
  return {
    ...
    ranges: [
      { oldStart: 1, oldEnd: 5, newStart: 1, newEnd: 10, changeType: 'modified' as const },
    ],
  };
}
...
describe('Heuristic Benchmarks', () => {
  it(
    'should analyze files with low latency',
    { timeout: 30_000 },
    async () => {
  ...
  it(
    'should analyze files with diff context',
    { timeout: 30_000 },
    async () => {
      ...
      console.log(
        `Analyze file with diff (all rules): mean=${formatMs(result.meanMs)}`,
      );
      expect(result.meanMs).toBeLessThan(20);
    },
  );
  ...
  it(
    ...
    async () => {
      ...
      console.log(
        `Analyze file (500 lines): mean=${formatMs(result.meanMs)}`,
      );
  ...
  it(
    'should throughput at scale (batch of files)',
    { timeout: 30_000 },
    async () => {
      ...
      console.log(
        `Batch analyze (100 files × 200 lines = 20K LOC): mean=${formatMs(result.meanMs)}`,
      );
    },
  );

  it(
    'should detect deep nesting on pathological input',
    { timeout: 10_000 },
    async () => {
  ...
  it(
    'should not regress on empty files',
    { timeout: 10_000 },
    async () => {
      const result = await bench(
        'analyze-empty-file',
