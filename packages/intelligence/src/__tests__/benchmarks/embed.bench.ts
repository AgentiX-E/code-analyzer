...
describe('Embedding Benchmarks', () => {
  it('should embed single snippets quickly', { timeout: 60_000 }, async () => {
    ...
    console.log(
      `Single embed (768d): mean=${formatMs(result.meanMs)}, p95=${formatMs(result.p95Ms)}`,
    );
  ...
  it('should embed small batches efficiently', { timeout: 60_000 }, async () => {
    const engine = new EmbeddingEngine({ dimensions: 768, normalize: true });
    await engine.initialize();

    const snippets = Array.from({ length: 10 }, (_, i) => generateCodeSnippet(5 + (i % 5)));
  ...
  it('should embed larger batches efficiently', { timeout: 60_000 }, async () => {
    const engine = new EmbeddingEngine({ dimensions: 768, normalize: true });
    await engine.initialize();

    const snippets = Array.from({ length: 100 }, (_, i) => generateCodeSnippet(3 + (i % 3)));
    let vectors: Float32Array[] = [];
  ...
  it('should compute cosine similarity quickly', { timeout: 30_000 }, async () => {
  ...
  it('should store and retrieve embeddings at scale', { timeout: 30_000 }, async () => {
