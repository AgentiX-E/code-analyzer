...
import { HybridSearchEngine, tokenize, cosineSimilarity } from '../../../src/search/hybrid-search.js';
...
function populateStore(store: InMemoryGraphStore, count: number): void {
  const methods = ['create', 'update', 'delete', 'find', 'list', 'process', 'validate', 'transform',
    'initialize', 'configure', 'handle', 'resolve', 'authenticate', 'authorize', 'parse',
    'serialize', 'deserialize', 'migrate', 'backup', 'restore'];
  const types = ['User', 'Post', 'Comment', 'Profile', 'Settings', 'Config', 'Auth', 'Session',
    'Role', 'Permission', 'Log', 'Metrics', 'Cache', 'Queue', 'Event'];
...
describe('Search Benchmarks', () => {
  it(
    'should build inverted index efficiently',
    { timeout: 60_000 },
    async () => {
  ...
  it(
    'should perform BM25 search with low latency',
    { timeout: 60_000 },
    async () => {
      const store = new InMemoryGraphStore();
  ...
  it(
    'should perform hybrid search with embeddings',
    { timeout: 60_000 },
    async () => {
      const store = new InMemoryGraphStore();
      ...
      searchEngine.registerEmbeddings(
        embedEngine.createEmbeddingLookup(),
        async (content: string) => embedEngine.embedCode(content),
      );
  ...
  it(
    'should fuse results efficiently',
    { timeout: 10_000 },
    async () => {
      const store = new InMemoryGraphStore();
  ...
  it(
    'should handle cache-friendly repeated searches',
    { timeout: 30_000 },
    async () => {
      const store = new InMemoryGraphStore();
