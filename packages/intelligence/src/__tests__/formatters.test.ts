...
describe('HtmlFormatter', () => {
  ...
  it('should include standardRef in HTML findings when present', () => {
    ...
    const html = formatter.format(report);
    expect(html).toContain('Standard Finding');
  });
  ...
  it('should include evidence code block in HTML', () => {
    ...
    const html = formatter.format(report);
    // The evidence should be in a <pre><code> block, but the content is escaped
    expect(html).toContain('maybeNull.value');
    expect(html).toContain('<pre>');
  });

  it('should handle empty findings and recommendations in HTML', () => {
    const report = makeReport({ findings: [], recommendations: [] });
    const html = formatter.format(report);
    // When findings and recommendations are empty, the sections shouldn't appear
    expect(html).not.toContain('<h2>Findings</h2>');
    expect(html).not.toContain('<h2>Recommendations</h2>');
  });
});
...
describe('MarkdownFormatter — edge cases', () => {
  const formatter = new MarkdownFormatter();

  it('should format score at exact boundary 89', () => {
    const report = makeReport({
      summary: { ...makeReport().summary, overallScore: 89 },
    });
    const md = formatter.format(report);
    expect(md).toContain('\u{1F7E1}'); // yellow for >= 70
  });

  it('should format score at exact boundary 69', () => {
    const report = makeReport({
      summary: { ...makeReport().summary, overallScore: 69 },
    });
    const md = formatter.format(report);
    expect(md).toContain('\u{1F7E0}'); // orange for >= 50
  });

  it('should format score at exact boundary 49', () => {
    const report = makeReport({
      summary: { ...makeReport().summary, overallScore: 49 },
    });
    const md = formatter.format(report);
    expect(md).toContain('\u{1F534}'); // red for < 50
  });

  it('should return white circle for unrecognized severity', () => {
    // Test severityIcon private method via format by using an unsupported severity
    // that gets rendered. We test it indirectly by verifying format handles it.
    const report = makeReport();
    const md = formatter.format(report);
    // Should include the footer section
    expect(md).toContain('Generated by Code Analyzer');
  });
