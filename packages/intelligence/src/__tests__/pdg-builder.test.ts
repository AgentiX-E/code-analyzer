...
function makeCfg(
  ...
  return {
    ...
    startColumn: 0,
    blocks: blocks.map((b) => ({
      index: b.idx, startLine: b.startLine, endLine: b.endLine,
      statementCount: b.stmts, isEntry: b.entry ?? false, isExit: b.exit ?? false,
    })),
    ...
    exitIndex: exitBlock,
    stmtFacts: {
      defs: new Map(), uses: new Map(),
      sourceSites: new Map(), sinkSites: new Map(), sanitizerSites: new Map(),
    },
  };
}
...
describe('buildPdg', () => {
  it('handles empty CFG', () => {
    const cfg: FunctionCfg = {
      functionName: 'empty', filePath: 'test.ts', startLine: 0, startColumn: 0,
      blocks: [], edges: [], bindings: [], entryIndex: -1, exitIndex: -1,
