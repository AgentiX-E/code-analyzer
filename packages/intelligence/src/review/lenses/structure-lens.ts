...
function countBranches(lines: string[]): number {
  ...
  for (const line of lines) {
    const t = line.trim();
    if (/\bif\b|\belse if\b|\?.*:|&&|\|\||\bcase\b|\bcatch\b|\bfor\b|\bwhile\b|\?.+\b/.test(t))
      count++;
  }
  return count;
}
...
function countMethods(lines: string[]): number {
  ...
  for (const line of lines) {
    if (
      /\b(?:function|async\s+function|static\s+(?:async\s+)?)\s*\w+\s*\(/.test(line) ||
      /\b\w+\s*\([^)]*\)\s*\{/.test(line)
    )
      count++;
  }
    ...
    for (const forbidden of sourceRule.forbiddenImports) {
      if (resolveLayer(importPath, layerRules) === forbidden) {
        ...
        const f = createLensFinding(
          ...
          evidence,
          {
            suggestion: `Move shared logic to a common layer or use dependency inversion.`,
            ruleId: 'struct-layer-violation',
          },
        );
...
function detectBarrelExports(lines: string[], filePath: string): LensFinding[] {
  ...
  if (reExportCount > 10) {
    ...
    const f = createLensFinding(
      ...
      evidence,
      {
        suggestion: 'Replace barrel exports with direct imports from individual module files.',
        ruleId: 'struct-barrel-export',
      },
    );
    if (f) findings.push(f);
  ...
  for (const node of funcNodes.items) {
    ...
    if (!hasIncomingCalls && !hasIncomingImports && !hasIncomingTests) {
      ...
      const f = createLensFinding(
        ...
        evidence,
        {
          suggestion: `Delete unused export "${node.name}" or add tests to confirm it's needed.`,
          ruleId: 'struct-orphan-code',
        },
      );
      if (f) findings.push(f);
...
export function analyzeStructure(
  ...
  if (complexity > 15) {
    ...
    const f = createLensFinding(
      ...
      evidence,
      {
        suggestion: 'Extract related logic into helper functions to reduce complexity.',
        ruleId: 'struct-complexity',
      },
    );
    if (f) findings.push(f);
  ...
  if (imports > 30) {
    ...
    const f = createLensFinding(
      ...
      evidence,
      {
        suggestion: 'Group related functionality into smaller, focused modules.',
        ruleId: 'struct-high-coupling',
      },
    );
    if (f) findings.push(f);
  ...
  for (const block of classBlocks) {
    if (block.lineCount > 500) {
      ...
      const f = createLensFinding(
        ...
        evidence,
        {
          suggestion: `Split ${block.name} into focused classes with single responsibilities.`,
          ruleId: 'struct-god-class-lines',
        },
      );
      if (f) findings.push(f);
    ...
    if (methodCount > 20) {
      ...
      const f = createLensFinding(
        ...
        evidence,
        {
          suggestion: `Extract related methods from ${block.name} into collaborator classes.`,
          ruleId: 'struct-god-class-methods',
        },
      );
      if (f) findings.push(f);
  ...
  for (const block of funcBlocks) {
    if (block.lineCount > 50) {
      ...
      const f = createLensFinding(
        ...
        evidence,
        {
          suggestion: `Extract sub-steps of ${block.name} into well-named helper functions.`,
          ruleId: 'struct-long-method',
        },
      );
      if (f) findings.push(f);
  ...
  if (depth > 4) {
    ...
    const f = createLensFinding(
      ...
      evidence,
      {
        suggestion: 'Use guard clauses (early returns) to reduce nesting.',
        ruleId: 'struct-deep-nesting',
      },
    );
    if (f) findings.push(f);
  ...
  if (cohesion < 0.3 && totalLines > 100) {
    ...
    const f = createLensFinding(
      ...
      evidence,
      {
        suggestion: 'Move unrelated functions to separate modules.',
        ruleId: 'struct-low-cohesion',
      },
    );
    if (f) findings.push(f);
