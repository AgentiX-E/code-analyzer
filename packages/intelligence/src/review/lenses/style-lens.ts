...
function detectAggregatedMagicNumbers(
  lines: string[],
  filePath: string,
): LensFinding[] {
  ...
  if (distinctMagic.size > 5) {
    ...
    const evidence: EvidenceAnchor = {
      ...
      lens: 'style', ruleId: 'style-magic-number-aggregated',
    };
    const f = createLensFinding('style', 'style', 'medium',
      ...
      evidence,
      { suggestion: `Extract magic numbers to const declarations or a constants file.`, ruleId: 'style-magic-number-aggregated' });
...
function tokenizeCode(lines: string[]): string[] {
  ...
  for (const line of lines) {
    const stripped = line.replace(/\/\/.*$/, '').replace(/\/\*[\s\S]*?\*\//g, '').trim();
  ...
  for (const [otherPath, otherContent] of repoFiles) {
    ...
    if (similarity >= 0.70) {
      ...
      const f = createLensFinding(
        ...
        evidence,
        { suggestion: `Extract common logic into a shared utility module imported by both files.`, ruleId: 'style-duplicate-code' });
      if (f) findings.push(f);
 ...
 */
function detectInconsistentNaming(
  lines: string[],
  filePath: string,
): LensFinding[] {
  const findings: LensFinding[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (varMatch) {
      ...
      let conv: string;
      if (/^[A-Z][A-Z0-9_]+$/.test(name)) conv = 'UPPER_CASE'; // constants
  ...
  if (varConvs && varConvs.size >= 2 && (varConvs.has('camelCase') && varConvs.has('snake_case'))) {
    const evidence: EvidenceAnchor = {
      ...
      lens: 'style', ruleId: 'style-inconsistent-naming',
    };
    const f = createLensFinding('style', 'style', 'low',
      ...
      evidence,
      { suggestion: 'Standardize on a single naming convention (e.g., camelCase for JavaScript/TypeScript).', ruleId: 'style-inconsistent-naming' });
    if (f) findings.push(f);
  }

  const funcConvs = conventions.get('function');
  if (funcConvs && funcConvs.size >= 2 && (funcConvs.has('camelCase') && funcConvs.has('snake_case'))) {
    const evidence: EvidenceAnchor = {
      filePath, startLine: 1, endLine: lines.length,
      codeSnippet: `Mixed function naming: ${[...funcConvs].join(', ')}`,
      lens: 'style', ruleId: 'style-inconsistent-function-naming',
    };
    const f = createLensFinding('style', 'style', 'low',
      ...
      evidence,
      { suggestion: 'Use camelCase consistently for all function names.', ruleId: 'style-inconsistent-function-naming' });
    if (f) findings.push(f);
  }
...
function detectCommentQuality(
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = i; j < lines.length; j++) {
      ...
      if (started && depth === 0) { funcEnd = j + 1; break; }
    }
    ...
    const f = createLensFinding('style', 'maintainability', 'medium',
      ...
      evidence,
      { suggestion: `Add JSDoc comment describing what ${funcName} does, its parameters, and return value.`, ruleId: 'style-missing-comment' });
    if (f) findings.push(f);
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/\bconsole\.log\b/.test(trimmed) && !trimmed.startsWith('//')) {
      const evidence: EvidenceAnchor = {
        ...
        lens: 'style', ruleId: 'style-console-log',
      };
      const f = createLensFinding('style', 'style', 'low',
        'Debug console.log',
 ...
 */
function detectFunctionMetrics(
  lines: string[],
  filePath: string,
): LensFinding[] {
  const findings: LensFinding[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (inFunction && braceDepth <= 0 && trimmed.includes('}')) {
      ...
      if (funcLength > 50) {
        const evidence: EvidenceAnchor = {
          filePath, startLine: funcStart, endLine: i + 1,
          codeSnippet: `${funcName}: ${funcLength} lines`,
          lens: 'style', ruleId: 'style-func-length',
        };
        const f = createLensFinding('style', 'style', 'medium',
          'Long Function',
      ...
      if (maxNestingDepth > 4) {
        const evidence: EvidenceAnchor = {
          ...
          lens: 'style', ruleId: 'style-nesting-depth',
        };
        const f = createLensFinding('style', 'style', 'medium',
          'Deep Nesting',
