...
export class MarkdownFormatter implements ReportFormatter {
  ...
  format(report: AnalysisReport): string {
    ...
    if (report.scope.type === 'pr' && report.scope.prNumber) {
      lines.push(
        `**PR:** #${report.scope.prNumber} (${report.scope.baseRef} → ${report.scope.headRef})  `,
      );
    }
    lines.push('');
    ...
    if (report.findings.length > 0) {
      ...
      for (const sev of severityOrder) {
        ...
        lines.push(
          `### ${icon} ${sev.charAt(0).toUpperCase() + sev.slice(1)} Severity (${bySeverity.length})`,
        );
        lines.push('');
    ...
    lines.push('');
    lines.push(
      `*Generated by Code Analyzer v${report.metadata.generatorVersion} on ${report.createdAt}*`,
    );
    lines.push('');
...
export class HtmlFormatter implements ReportFormatter {
  ...
  format(report: AnalysisReport): string {
    ...
    lines.push(
      `<tr><td>Risk Level</td><td class="risk-${report.summary.riskLevel}">${report.summary.riskLevel.toUpperCase()}</td></tr>`,
    );
    ...
    lines.push(
      `<tr><td>Merge Recommendation</td><td><strong>${report.summary.mergeRecommendation}</strong></td></tr>`,
    );
    lines.push('</table>');
    ...
    if (report.findings.length > 0) {
      ...
      for (const finding of report.findings) {
        ...
        lines.push(
          `<strong>${finding.severity.toUpperCase()}</strong> — ${this.escapeHtml(finding.title)}`,
        );
        lines.push('</div>');
        ...
        if (finding.lineRange) {
          lines.push(
            `<p><strong>Lines:</strong> L${finding.lineRange[0]}-L${finding.lineRange[1]}</p>`,
          );
        }
    ...
    if (report.recommendations.length > 0) {
      ...
      for (const rec of report.recommendations) {
        ...
        lines.push(
          `<p><strong>Priority:</strong> ${rec.priority} | <strong>Effort:</strong> ${rec.estimatedEffort} | <strong>Files:</strong> ${rec.affectedFiles.length}</p>`,
        );
        if (rec.description) {
    ...
    lines.push('<hr>');
    lines.push(
      `<p class="footer">Generated by Code Analyzer v${report.metadata.generatorVersion}</p>`,
    );
    lines.push('</div>');
    lines.push('</body>');
    lines.push('</html>');
