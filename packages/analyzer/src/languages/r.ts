...
export class RProvider extends TreeSitterBaseProvider {
  ...
  protected override walkAndCapture(node: TreeSitterSyntaxNode, captures: UnifiedCapture[]): void {
    ...
    if (nt === 'function_definition') {
      ...
      if (!funcName) {
        // Anonymous function_definition — its name lives on the enclosing
        // assignment's identifier, not on this node, so fall back to fn_<line>.
        funcName = `fn_${node.startPosition.row + 1}`;
      }
