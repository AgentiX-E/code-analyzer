...
describe('Graph Benchmarks', () => {
  it('should build empty graph quickly', { timeout: 30_000 }, async () => {
  ...
  it('should add nodes at high throughput', { timeout: 30_000 }, async () => {
    const store = createStore();
    const builder = new GraphBuilder(store);
    ...
    console.log(
      `Add 1000 nodes: mean=${stats.duration.mean.toFixed(2)}ms, nodes/sec=${(1000 / (stats.duration.mean / 1000)).toFixed(0)}`,
    );
  });

  it('should add edges at high throughput', { timeout: 30_000 }, async () => {
    const store = createStore();
    const builder = new GraphBuilder(store);
    ...
    expect(stats.duration.mean).toBeLessThan(500);
    console.log(
      `Add 1000 edges: mean=${stats.duration.mean.toFixed(2)}ms, edges/sec=${(1000 / (stats.duration.mean / 1000)).toFixed(0)}`,
    );
  });

  it('should validate large graphs efficiently', { timeout: 30_000 }, async () => {
    const store = createStore();
    const builder = new GraphBuilder(store);
  ...
  it('should dump to store efficiently', { timeout: 30_000 }, async () => {
    const store = createStore();
    const builder = new GraphBuilder(store);
