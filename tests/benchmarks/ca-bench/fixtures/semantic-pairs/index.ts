...
export const HIGH_SIMILARITY_PAIRS: EmbeddingPair[] = [
  ...
  },
  {
    ...
    textB:
      'async function getUser(userId) { const resp = await fetch(`/api/users/${userId}`); return resp.json(); }',
    expectedSimilarity: 'high',
    minCosSimilarity: 0.85,
  },
  {
    id: 'hs-04',
    textA:
      'class UserService { async findById(id: number): Promise<User> { return db.users.findOne({ id }); } }',
    textB:
      'class UserRepository { async findById(id: number): Promise<User> { return this.db.users.findOne({ id }); } }',
    expectedSimilarity: 'high',
    minCosSimilarity: 0.8,
  },
];
...
export const MEDIUM_SIMILARITY_PAIRS: EmbeddingPair[] = [
  {
    ...
    minCosSimilarity: 0.5,
  },
  {
    id: 'ms-02',
    textA:
      'SELECT * FROM users WHERE email = ? AND active = true ORDER BY created_at DESC LIMIT 10',
    textB:
      'SELECT id, name FROM users WHERE email = ? AND active = 1 ORDER BY created_at DESC LIMIT 10',
    expectedSimilarity: 'medium',
    maxCosSimilarity: 0.9,
    minCosSimilarity: 0.6,
  },
  {
    id: 'ms-03',
    textA:
      'const server = express(); server.use(cors()); server.use(express.json()); server.get("/health", (req, res) => res.json({ status: "ok" }));',
    textB:
      'app.use(cors()); app.use(bodyParser.json()); app.get("/ping", (req, res) => res.send({ status: "ok" }));',
    expectedSimilarity: 'medium',
    maxCosSimilarity: 0.85,
...
export const LOW_SIMILARITY_PAIRS: EmbeddingPair[] = [
  ...
  },
  {
    id: 'ls-03',
    textA:
      'public class PaymentProcessor { public void process(Payment p) { gateway.charge(p); } }',
    textB:
      'public class ImageResizer { public void resize(Image img, int w, int h) { return img.scale(w, h); } }',
    expectedSimilarity: 'low',
    maxCosSimilarity: 0.5,
  },
  {
    id: 'ls-04',
    textA:
      'terraform { required_providers { aws = { source = "hashicorp/aws" } } } resource "aws_instance" "web" { ami = "ami-123" }',
    textB: '#include <stdio.h>\nint main() { printf("Hello, World!\\n"); return 0; }',
    expectedSimilarity: 'low',
    maxCosSimilarity: 0.3,
  },
];
